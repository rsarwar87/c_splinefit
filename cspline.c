...
cspline_alloc (size_t size)
{
  ...
  if (state == NULL)
    {
      fprintf(stderr, "failed to allocate space for state");
    }
  ...
  if (state->c == NULL)
    {
      free (state);
      fprintf(stderr,"failed to allocate space for c");
    }
  ...
  if (state->g == NULL)
    {
      free (state->c);
      free (state);
      fprintf(stderr,"failed to allocate space for g");
    }
  ...
  if (state->diag == NULL)
    {
      ...
      free (state->c);
      free (state);
      fprintf(stderr,"failed to allocate space for diag");
    }
  ...
  if (state->offdiag == NULL)
    {
      ...
      free (state->c);
      free (state);
      fprintf(stderr,"failed to allocate space for offdiag");
    }
