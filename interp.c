...
gsl_interp_alloc (const gsl_interp_type * T, size_t size)
{
  ...
  if (size < T->min_size)
    {
      fprintf (stderr, "insufficient number of points for interpolation type");
    }
  ...
  if (interp == NULL)
    {
      fprintf (stderr, "failed to allocate space for interp struct");
    }
  ...
  if (interp->state == NULL)
    {
      free (interp);          
       fprintf (stderr, "failed to allocate space for interp state");
    };
...
gsl_interp_init (gsl_interp * interp, const double x_array[], const double y_array[], size_t size)
{
  ...
  if (size != interp->size)
    {
       fprintf (stderr, "data must match size of interpolation object");
    }
  ...
  for (i = 1; i < size; i++) 
    {
      if (!(x_array[i-1] < x_array[i])) 
        {
           fprintf (stderr, "x values must be strictly increasing");
        }
    }
                 ...
                 gsl_interp_accel * a)
{
  ...
  if (status != 0) fprintf (stderr, "interpolation error");
