  ...
  size_t N)
{
  ...
  if (gamma == 0 || alpha == 0 || c == 0 || z == 0)
    {
      fprintf(stderr,"failed to allocate working space");
    }
  else
    {
      size_t i, j;
  ...
  gsl_vector * solution)
{
  if(diag->size != rhs->size)
    {
      fprintf(stderr,"size of diag must match rhs");
    }
  else if (offdiag->size != rhs->size-1)
    {
      fprintf(stderr,"size of offdiag must match rhs-1");
    }
  else if (solution->size != rhs->size)
    {
      fprintf(stderr,"size of solution must match rhs");
    }
  else 
    {
